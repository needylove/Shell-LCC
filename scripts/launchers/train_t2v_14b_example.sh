...
#   - use the BAND reward (--reward_target): the plain reward overshoots through the data shell
#     in ~10 steps; targeting |dist - tau| (tau = the logged reward of a known-good checkpoint)
#     roughly doubles both the usable window (~20 steps) and the detail gain.
#   - reward hacking still accumulates with steps (noise on flat backgrounds) — save every 5
#     steps, stop early, and INSPECT FRAMES before picking a checkpoint.
#   - lower lr (e.g. 5e-6 / 8e-6) does not help on 14B — it never reaches the improvement zone.
...
TARGET="${TARGET:-0.444}"    # band target; set 0 for the plain reward
